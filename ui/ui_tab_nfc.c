...
static void nfc_update_cb(lv_timer_t *t) {
    ...
    if (g_nfc.tag_detected) {
        ...
        lv_obj_set_style_text_color(uid_label, lv_color_hex(0x555555), 0);
    } else {
...
void ui_tab_nfc_create(lv_obj_t *parent) {
    ...
    lv_obj_set_style_bg_color(conn_bar, lv_color_hex(0xE8E8E8), 0);
    ...
    lv_obj_set_style_bg_color(line, lv_color_hex(0xD0D0DD), 0);
